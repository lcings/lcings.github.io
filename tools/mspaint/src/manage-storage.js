...
function manage_storage(){
	...
	$storage_manager = $FormWindow().title("内存管理").addClass("storage-manager dialogue-window");
	...
	var $message = $(E("p")).appendTo($storage_manager.$main).html(
		"您保存到您的计算机上的任何图像，包括 文件>保存 将不会受到影响."
	);
	$storage_manager.$Button("关闭", function(){
		$storage_manager.close();
	});
	...
	var addRow = function(k, imgSrc){
		...
		var $remove = $(E("button")).text("清除").addClass("remove-button");
